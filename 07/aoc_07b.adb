...
begin
   ...
   --  cost of the distance is (n*(n+1))/2
   Costs := (others => 0);
   for Pos in Costs'Range loop
      for P in Crabs'Range loop
         declare
            Dist : constant Long_Long_Integer := Long_Long_Integer (abs (Pos - P));
            Cost : constant Long_Long_Integer := (Dist * (Dist+1))/2;
         begin
            Costs(Pos) := Costs(Pos) + Cost*Crabs(P);
         end;
      end loop;
